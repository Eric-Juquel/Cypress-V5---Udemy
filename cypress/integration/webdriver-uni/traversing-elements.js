...
describe("Traversing DOM elements in Cypress", () => {
  ...
  it("filter() to retrieve DOM elements that match a specific selector", () => {
    cy.get('.btn-group > *').filter('.active').should("have.css", "background-color", "rgb(40, 96, 144)")
  });
  ...
  it("not() to remove DOM element(s) from the set of elements", () => {
    cy.get('.traversal-button-states > button').not('.disabled').should('not.have.css','opacity', .65)
  });
  ...
  it.only("siblings() To get all sibling DOM elements of elements", () => {
    cy.get('.btn-group > .active').siblings().should('have.length', 3)
  });
});
